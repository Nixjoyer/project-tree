...
Example output generated by ProjTree:
...
Ignore certain directories or files by name:

```bash
projtree --ignore node_modules,__pycache__
```

Enable watch mode (runs until interrupted):

```bash
projtree --watch
```

Watch mode without initial generation:
